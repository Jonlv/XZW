...
@implementation XZWNearByFriendsView
@synthesize delegate;
...
- (id)initWithFrame:(CGRect)frame andLinkString:(NSString *)linkStringArg {
	...
	if (self) {
        ...
        //
        //
        //        resolveRequest = [[ASIHTTPRequest  alloc ]   initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@&p=%d&ssex=%d&lat=%f&lng=%f",linkPreString,current,sex,[[NSUserDefaults standardUserDefaults] doubleForKey:@"userLatitude"],[[NSUserDefaults standardUserDefaults] doubleForKey:@"userLongitude"] ]]];
        //
        //
        //        NSLog(@"resolveRequest %@" ,[[resolveRequest  url]  absoluteString]);
        //
        //
...
- (void)loadNewPage {
	...
	resolveRequest = [[ASIHTTPRequest alloc] initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@&p=%d&ssex=%d&lat=%f&lng=%f", linkPreString, current + 1, sex, [[NSUserDefaults standardUserDefaults] doubleForKey:@"userLatitude"], [[NSUserDefaults standardUserDefaults] doubleForKey:@"userLongitude"]]]];
	...
	[resolveRequest setCompletionBlock: ^{
	    ...
	    NSLog(@"%@~~~", [[resolveRequest responseData]   objectFromJSONData]);
...
- (void)reloadFirst {
	...
	[resolveRequest setCompletionBlock: ^{
	    ...
	    if ([[[[resolveRequest responseData]   objectFromJSONData] objectForKey:@"status"]  intValue] == 0) {
	        ...
	        if ([[[[resolveRequest responseData]   objectFromJSONData] objectForKey:@"data"] isKindOfClass:[NSDictionary class]]) {
	            totalPage = [[[[[resolveRequest responseData]   objectFromJSONData] objectForKey:@"data"] objectForKey:@"totalPages"]  intValue];

	            thisCount = [[[[[resolveRequest responseData]   objectFromJSONData] objectForKey:@"data"] objectForKey:@"data"]  count];


	            [zhiDaoArray setArray:[[[responseData   objectFromJSONData] objectForKey:@"data"] objectForKey:@"data"]];
			}
	        else {
	            thisCount = [[[[resolveRequest responseData]   objectFromJSONData] objectForKey:@"data"]   count];


	            [zhiDaoArray setArray:[[[resolveRequest responseData]   objectFromJSONData] objectForKey:@"data"]];
			}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (cell == nil) {
        ...
        //        UILabel *distanceUL = [[UILabel alloc]  initWithFrame:CGRectMake(95, 3, 230, 30)];
        //        distanceUL.tag = 3336;
        //        distanceUL.backgroundColor = [UIColor clearColor];
        //        distanceUL.numberOfLines = 1;
        //        distanceUL.highlightedTextColor = [UIColor whiteColor];
        //        [cell.contentView addSubview:distanceUL];
        //        [distanceUL release];
		//♂  ♀
		...
		// me distance
		myLabel = [[UILabel alloc] initWithFrame:CGRectMake(182, 8, 130, 20)];
		myLabel.tag = 3336;
		myLabel.backgroundColor = [UIColor clearColor];
		myLabel.numberOfLines = 1;
	...
	if (![[zhiDaoArray objectAtIndex:indexPath.row] objectForKey:@"sex"]) {
		[cell.contentView viewWithTag:2222].hidden = true;
	}

	[(UILabel *)[cell.contentView viewWithTag:3335]  setText :[[zhiDaoArray objectAtIndex:indexPath.row] objectForKey:@"uname"]];
	[(UILabel *)[cell.contentView viewWithTag:3334]  setText :[[zhiDaoArray objectAtIndex:indexPath.row] objectForKey:@"intro"] ? [[zhiDaoArray objectAtIndex:indexPath.row] objectForKey:@"intro"] :[[zhiDaoArray objectAtIndex:indexPath.row] objectForKey:@"content"]];

	[(UILabel *)[cell.contentView viewWithTag:3336]  setText : [[zhiDaoArray objectAtIndex:indexPath.row] objectForKey:@"distance"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (delegate) {
		if ([delegate respondsToSelector:@selector(selectNearByID:)]) {
			[delegate selectNearByID:[[[zhiDaoArray objectAtIndex:indexPath.row] objectForKey:@"uid"] intValue]];
		}
	}
}
