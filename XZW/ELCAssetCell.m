...
- (void)cellTapped:(UITapGestureRecognizer *)tapRecognizer
{
	...
	for (int i = 0; i < [_rowAssets count]; ++i) {
        if (CGRectContainsPoint(frame, point)) {
            ...
            if (!asset.selected &&
                [[NSUserDefaults standardUserDefaults]   integerForKey:@"uploadLimit" ] >= TotalLimit ) {
                
                UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"" message:[NSString stringWithFormat:@"一次最多能上传%d张图片",TotalLimit] delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
                [alert show];
                [alert release];
