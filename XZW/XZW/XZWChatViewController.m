...
@interface XZWChatViewController ()<UITableViewDataSource,UITableViewDelegate,IIViewDeckControllerDelegate,UITextFieldDelegate>{
...
@implementation XZWChatViewController
...
-(void)getFriendChat{
    ...
    for (int i = [chatArray  count] - 1; i >=0 ; i--) {
        ...
        if (chatArray[i][@"message_id"] ) {
            
            lastMessageID = [chatArray[i][@"message_id"] intValue]  ;
    ...
    if (lastMessageID == 0) {
        ...
        if (listID == -1) {
            ...
            lastMessageID = [XZWDBOperate getMaxIDFromUserID:userID];
            
        }else {
            
            lastMessageID = [XZWDBOperate getMaxIDFromListID:listID];
...
//        friendChatLink = [XZWLoadSiXin  stringByAppendingFormat: @"&list_id=%d&isPoll=1&message_id=%d",listID,lastMessageID ];
        
        
        
        if (listID < 0 ) {
            
            friendChatLink = [XZWLoadSiXin  stringByAppendingFormat:@"&uid=%d&isPoll=1&message_id=%d",userID,lastMessageID];
       ...
       // friendChatLink =[XZWLoadSiXin  stringByAppendingFormat: @"&list_id=%d&isPoll=1",listID];
        
        
        
        if (listID < 0 ) {
            
            friendChatLink = [XZWLoadSiXin  stringByAppendingFormat:@"&uid=%d&isPoll=1",userID];
    ...
    getChatRequest = [XZWNetworkManager asiWithLink:friendChatLink   postDic:nil completionBlock:^{
        ...
        if ([tempDic[@"status"]  intValue] == 1) {
            ...
            [chatArray    addObjectsFromArray:tempDic[@"data"][@"data"]];
            [chatTableView reloadData];
