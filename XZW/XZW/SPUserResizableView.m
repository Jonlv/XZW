...
#define kSPUserResizableViewDefaultMinWidth (320.f / 3)
#define kSPUserResizableViewDefaultMinHeight (155.f / 3)
#define kSPUserResizableViewInteractiveBorderSize 10.0

#define kRatio  (320.f / 155.f)
...
- (void)resizeUsingTouchLocation:(CGPoint)touchPoint {
    ...
    CGFloat deltaW,deltaX,deltaH,deltaY;
     
    
    if (anchorPoint.adjustsW != 0 ) {
        
         deltaW = anchorPoint.adjustsW * (touchStart.x - touchPoint.x)  ;
         deltaX = anchorPoint.adjustsX * (-1.0 * deltaW) ;
         deltaH =  deltaW / kRatio ;
         deltaY = anchorPoint.adjustsY * (-1.0 * deltaH) ;
         
        
    }else {
        
        
        deltaH = anchorPoint.adjustsH * (touchPoint.y - touchStart.y) ;
         deltaW = deltaH  * kRatio  ;
         deltaX = anchorPoint.adjustsX * (-1.0 * deltaW) ;
         deltaY = anchorPoint.adjustsY * (-1.0 * deltaH) ;
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(userResizableViewDidMoving:)]) {
        [self.delegate userResizableViewDidMoving:self];
    }
