...
- (void)_setupNavigationBar{
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"选择", @"")
...
///////////
        ...
        [self.useButton setTitle:NSLocalizedString(@"选择",@"") forState:UIControlStateNormal];
...
- (void)_setupToolbar{
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
        ...
        if ([[[UIDevice currentDevice]  systemVersion] floatValue] > 6.0) {
            ...
            info.text = NSLocalizedString(@"移动和缩放", @"");
        }
