...
@implementation XZWQuanIssueTopCell
@synthesize delegate;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        imageButton.frame = CGRectMake(40, 40, 100, 100);
...
-(CGFloat)setAvatarURLString:(NSString*)urlString name:(NSString*)nameString description:(NSString*)commentString date:(NSString*)dateString andTag:(int)index andSelect:(BOOL)selected picImage:(NSString*)imageString{
    ...
    float height = CGRectGetMaxY(contentUL.frame);
    
    if (imageString) {
        
        [imageButton  setImageWithURL:[NSURL URLWithString:imageString] forState:UIControlStateNormal];
        imageButton.hidden = false;
        imageButton.frame = CGRectMake(60, height + 10  , 100, 100);
        
        
         height = CGRectGetMaxY(imageButton.frame);
    ...
    dateUL.frame = CGRectMake(60, height + 8 , 300, 20);
    
    commentButton.frame = CGRectMake(290, height + 13, 15 , 13);
    commentButton.tag = index;
    
    likeButton.frame = CGRectMake(266, height + 11, 15 , 13);
    likeButton.selected = selected;
    likeButton.tag = index ;
...
-(void)imageAction:(UIButton*)sender{
    ...
    if (delegate) {
        ...
        if ([delegate respondsToSelector:@selector(clickPicIndex:)]) {
            
            [delegate clickPicIndex:sender.tag];
        }
